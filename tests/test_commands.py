...
from .conftest import skip_if_server_version_lt, skip_if_server_version_gte, with_ns
...
class TestRedisCommands(object):
    ...
    def test_slowlog_get_limit(self, r, slowlog, ns):
        assert r.slowlog_reset()
        r.get('foo')
        r.get('bar')
        ...
        assert isinstance(slowlog, list)
        commands = [log['command'] for log in slowlog]
        assert b('GET %sfoo' % ns) not in commands
        assert b('GET %sbar' % ns) in commands
    ...
    def test_randomkey(self, r):
        ...
        for key in ('a', 'b', 'c'):
            r[key] = 1
        assert r.randomkey() in (b(with_ns('a')), b(with_ns('b')), b(with_ns('c')))
