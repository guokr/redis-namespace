...
if not redis_version.startswith(".".join(current_version.split(".")[:-1])):
    raise Exception(
        "Version mismatch! redis version: %s, redis-namespace version: %s"
...
def args_with_namespace(ns, *original_args):
    ...
    if before == "first":
        ...
        for i, a in enumerate(args):
            if isinstance(a, (str, bytes)) and str(a).lower() == "match":
    ...
    def sort(
        ...
        if get:
            if isinstance(get, (str, bytes)):
                ...
                get = [
                    add_namespace(self._namespace, i) if i != "#" else i for i in get
                ]
...
class Pipeline(_Pipeline, StrictRedis):
    def __init__(
        self, connection_pool, response_callbacks, transaction, shard_hint, namespace=""
    ):
